...
//		command_alert("The station has entered an ion storm.  Monitor all electronic equipment for malfunctions", "Anomaly Alert")
	...
	laws |=         list( "You must always lie.", "All answers must be in the form of a question.", "The NSS Dawn is a ship, not a station. You are now assigned to a ship, instead of a station." )
	...
	if(message_servers)
		for (var/obj/machinery/message_server/MS in message_servers)
			...
			for (i = 1, i <= MS.spamfilter_limit, i++)
				MS.spamfilter += pick("kitty","HONK","rev","malf","liberty","freedom","drugs", "Dawn", \
